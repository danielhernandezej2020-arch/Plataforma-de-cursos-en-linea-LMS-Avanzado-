    ...
    >
      ...
      {log.response !== undefined && log.response !== null && (
        ...
        >
          {(() => {
            const responseText =
              JSON.stringify(log.response, null, 2) ?? String(log.response);
    ...
    <div style={{ animation: "fadeUp 0.6s ease both" }}>
      ...
      >
        ...
        {["0,0", "auto,0", "0,auto", "auto,auto"].map((pos, i) => {
          const [t, b] = pos.includes("auto,auto")
            ? ["auto", "0"]
            : pos.includes("0,auto")
              ? ["auto", "0"]
              : ["0", "auto"];
          const [l, r] =
            pos.includes(",0") && i < 2 ? ["0", "auto"] : ["auto", "0"];
          return (
            <div
              key={i}
              style={{
        ...
        <div
          style={{
            animation:
              "stamp-in 0.5s 0.3s cubic-bezier(0.175,0.885,0.32,1.275) both",
            marginBottom: 24,
          }}
        >
          <div
...
function QuizUI({
  ...
  const [answers, setAnswers] = useState<number[]>(
    new Array(evaluation.questions.length).fill(-1),
  );
...
export default function LMSApp({
  ...
  const log = useCallback(
    (
      ...
      setApiLog((prev) => [
        ...prev,
        { method, url, status, response, ts: Date.now(), pattern },
      ]);
    },
    [],
  );
  ...
  const doEnroll = useCallback(
    async (course: Course, pmt: Payment | null) => {
        ...
        const evalData = (await api(
          ...
          "/api/evaluations",
          {
            ...
            questions: isQuiz
              ? [
                  {
                    question: `What is the main benefit of ${course.title.split(" ")[0]}?`,
                    options: [
                      "Speed",
                      "Type safety",
                      "Simplicity",
                  ...
                  {
                    question: "Which pattern ensures a single instance?",
                    options: ["Factory", "Observer", "Singleton", "Decorator"],
                    correctIndex: 2,
                  },
                  {
                    question: "Clean Architecture separates concerns into:",
                    options: ["Files", "Layers", "Functions", "Classes"],
                    correctIndex: 1,
                  },
                ]
              : [
  ...
  const submitQuiz = useCallback(
    async (answers: number[]) => {
      ...
      try {
        const sub = (await api(
          "POST",
          `/api/evaluations/${evaluation.id}/submit`,
          {
            userId: demoUser.id,
            answers,
          },
    ...
    try {
      const sub = (await api(
        ...
        `/api/evaluations/${evaluation.id}/submit`,
        {
          userId: demoUser.id,
          answers: {
            projectUrl: "https://github.com/alex-chen/capstone",
            notes: "Implemented all patterns",
          },
        },
      )) as Submission;
      setSubmission(sub);
  ...
  const getRecs = useCallback(async () => {
    ...
    try {
      const recs = (await api(
        "GET",
        `/api/recommendations?userId=${demoUser.id}`,
        undefined,
        "Singleton",
      )) as Recommendation[];
  ...
  return (
    <div style={{ position: "relative", zIndex: 1, minHeight: "100vh" }}>
      ...
      >
        ...
        >
          <div>
            ...
            >
              Production-grade LMS with{" "}
              <strong style={{ color: "var(--text)" }}>Singleton</strong> &amp;{" "}
              <strong style={{ color: "var(--text)" }}>Factory Method</strong>{" "}
              patterns wired into every layer. Watch them run live.
            </p>
            <div
              style={{
                display: "flex",
        ...
        >
          {filteredCourses.map((course, i) => (
            <div
              ...
              onMouseLeave={(e) => {
                (e.currentTarget as HTMLDivElement).style.borderColor =
                  step === "select" ? "var(--border-accent)" : "var(--border)";
                (e.currentTarget as HTMLDivElement).style.transform =
                  "translateY(0)";
              }}
            >
      ...
      <section
        id="demo"
        ref={demoRef}
        style={{ padding: "0 40px 100px", maxWidth: 1100, margin: "0 auto" }}
      >
        <div
          style={{
            display: "flex",
        ...
        {step === "idle" ? (
          ...
          >
            ...
            >
              Creates a real user in PostgreSQL, processes payments through the
              Factory pattern, enforces Singleton idempotency, generates
              certificates — all live.
            </p>
            <button
              onClick={startDemo}
              disabled={loading}
          ...
          >
            ...
            >
              ...
              {steps
                .filter((s) =>
                  selectedCourse?.type === "free" ? s.key !== "paying" : true,
                )
                .map((s, i) => {
            ...
            >
              ...
              {step === "select" && (
                <div style={{ animation: "fadeUp 0.4s ease both" }}>
                  ...
                  >
                    Click any course card above, or select one below. Free
                    courses skip payment.
                  </p>
                  <div
                    style={{
                      display: "flex",
              ...
              {step === "paying" && selectedCourse && (
                <div style={{ animation: "fadeUp 0.4s ease both" }}>
                  ...
                  >
                    Payment provider (Factory creates the implementation):
                  </p>
                  ...
                  >
                    {loading
                      ? "Processing…"
                      : `Pay $${selectedCourse.price} with ${payProvider}`}
                  </button>
                </div>
              )}
              ...
              {step === "enrolling" && (
                ...
                >
                  ...
                  <p style={{ color: "var(--muted)", fontSize: 14 }}>
                    Enrolling and creating evaluation…
                  </p>
                </div>
              )}
              ...
              {step === "quiz" &&
                ...
                (evaluation.type === "quiz" ? (
                  ...
                  <div style={{ animation: "fadeUp 0.4s ease both" }}>
                    ...
                    >
                      Submit your project URL. Score is auto-simulated at 75%
                      (passing = 60%).
                    </p>
                    <div
                      style={{
                    ...
                    {(
                      evaluation.questions as unknown as {
                        criteria: string;
                        maxScore: number;
                      }[]
                    ).map((r, i) => (
                ...
                >
                  <div
                    style={{
                      ...
                      fontSize: 36,
                      animation:
                        "stamp-in 0.5s cubic-bezier(0.175,0.885,0.32,1.275) both",
                    }}
                  >
                    {submission.passed ? "🏆" : "✗"}
                  </div>
              ...
              {step === "recs" && (
                <div style={{ animation: "fadeUp 0.4s ease both" }}>
                  ...
                  {recommendations.length === 0 ? (
                    ...
                    >
                      No recommendations yet — enroll in more courses to train
                      the engine! 🎯
                    </div>
                  ) : (
                    <div
                  ...
                  >
                    ...
                    >
                      All patterns executed: 4 Singletons + 4 Factory Methods
                    </p>
                    <button
                      onClick={resetDemo}
                      style={{
